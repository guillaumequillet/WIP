...
module Config
  ...
  FADE_SPEED = 15 # Vitesse du fondu (plus c'est haut, plus c'est rapide)
end
...
class CollisionBox
  ...
  def draw_debug
    glColor4f(1, 0, 0, 0.7); glLineWidth(2)
    glBegin(GL_LINE_LOOP)
      glVertex3f(@x_min, @y_min, 0.1); glVertex3f(@x_max, @y_min, 0.1)
      glVertex3f(@x_max, @y_max, 0.1); glVertex3f(@x_min, @y_max, 0.1)
    glEnd
  end
end
...
class TriggerZone
  ...
  def draw_debug
    glColor4f(0, 1, 0, 0.7)
    glBegin(GL_LINE_LOOP)
      glVertex3f(@x_min, @y_min, 0.2); glVertex3f(@x_max, @y_min, 0.2)
      glVertex3f(@x_max, @y_max, 0.2); glVertex3f(@x_min, @y_max, 0.2)
    glEnd
  end
end
...
class Entity
  ...
  def update(camera, collisions, locked)
    return if locked # Empêche Jill de bouger pendant le fondu
    ...
    if @moving
      ...
      mv_x = Math.cos(@angle) * Config::JILL_SPEED
      mv_y = Math.sin(@angle) * Config::JILL_SPEED
      @x += mv_x unless collisions.any? { |b| b.hit?(@x + mv_x, @y, Config::JILL_RADIUS) }
      @y += mv_y unless collisions.any? { |b| b.hit?(@x, @y + mv_y, Config::JILL_RADIUS) }
    end
...
class Prototype < Gosu::Window
  ...
  def update
    # Mise à jour de Jill (mouvement bloqué si on fade)
    @jill.update(@cameras[@idx], @collisions, @fade_state != :none)
    
    case @fade_state
    when :none
      # Détection normale des zones
      ...
      if all_active.any?
        ...
        unless all_active.any? { |z| z.cam_id == current_cam_id }
          ...
          @next_cam_idx = @cameras.find_index { |c| c.id == new_cam_id }
          @fade_state = :out if @next_cam_idx
        end
      end
    when :out
      @fade_alpha += Config::FADE_SPEED
      if @fade_alpha >= 255
        @fade_alpha = 255
        @idx = @next_cam_idx
        @fade_state = :in
  ...
  def draw
    ...
    # 3. Le rectangle de fondu (Overlay 2D)
    if @fade_alpha > 0
      draw_rect(0, 0, width, height, Gosu::Color.new(@fade_alpha, 0, 0, 0), 100)
    end
  end

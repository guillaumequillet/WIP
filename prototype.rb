...
class Entity
  ...
  def update(camera, collisions)
    ...
    if @moving
      # Calcul de l'angle d'input relatif à l'écran
      input_angle = Math.atan2(dy_screen, dx_screen)
      
      # Direction finale = Yaw Caméra + Input - Offset 90° (OpenGL Forward)
    ...
    # Détermination visuelle du sprite (Toujours relatif à la caméra)

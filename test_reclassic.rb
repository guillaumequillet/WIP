...
module Config
  ...
  # --- CHANGEMENT ICI ---
  # Rayon beaucoup plus large pour bien protéger le sprite des murs
  JILL_RADIUS = 2.5 
end
...
class Entity
  ...
  def initialize(file, x, y)
    ...
    # OMBRE ULTRA-BASSE RÉSOLUTION (8x8) - Inchangé
  ...
  def draw(camera, debug)
    ...
    glPushMatrix
      ...
      draw_shadow
      if @dir == :face; draw_beam; draw_sprite(info, camera)
      else; draw_sprite(info, camera); draw_beam; end
  ...
  def draw_shadow
    ...
    # --- CHANGEMENT ICI ---
    # Taille réduite de moitié (0.28 / 2 = 0.14)
  ...
  def draw_beam
    ...
    glPushMatrix; glRotatef(@angle * 180.0 / Math::PI, 0, 0, 1)
      glBegin(GL_TRIANGLE_FAN)
        glColor4f(1, 1, 0.7, 0.12); glVertex3f(0, 0, 3)
        glColor4f(1, 1, 0.7, 0.0)
        17.times { |i| a = i * 2.0 * Math::PI / 16.0; glVertex3f(48, 12 * Math.cos(a), 3 + 12 * Math.sin(a)) }
      glEnd
